...
//    if ([sender tag]==choosedTab) return;
    ...
    self.navigationItem.title = [NSString stringWithFormat:NSLocalizedString(@"More (%ld)", nil), (count - MAX_NORMAL_BUTTONS)];
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad){
        [UIView beginAnimations:nil context:nil];
        [UIView setAnimationDuration:0.3];
        topNavigationLabel.alpha = 0;
        [UIView commitAnimations];
        topNavigationLabel.text = [NSString stringWithFormat:NSLocalizedString(@"More (%ld)", nil), (count - MAX_NORMAL_BUTTONS)];
        [UIView beginAnimations:nil context:nil];
...
//AB    UISearchBarLeftButton *bar = (UISearchBarLeftButton *)self.searchDisplayController.searchBar;
...
//AB        [bar.leftButton setImage:[UIImage imageNamed:@"button_view.png"] forState:UIControlStateNormal];
    }
    else{
...
//AB        self.searchController.searchBar.tintColor = [utils lighterColorForColor:tableViewSearchBarColor];
//AB        searchBarColor = tableViewSearchBarColor;
//AB        [bar.leftButton setImage:[UIImage imageNamed:@"button_view_list.png"] forState:UIControlStateNormal];
    }
    if (!isViewDidLoad){
...
//AB    UISearchBarLeftButton *bar = (UISearchBarLeftButton *)self.searchDisplayController.searchBar;
//AB    [bar showLeftButton:NO];
//AB    [bar showSortButton:NO];
//AB    if ([self collectionViewCanBeEnabled] == YES){
//AB        [bar showLeftButton:YES];
//AB    }
    sortMethodIndex = -1;
    sortMethodName = nil;
    sortAscDesc = nil;
//AB    if ([[[parameters objectForKey:@"parameters"] objectForKey:@"sort"] objectForKey:@"available_methods"] != nil) {
//AB       [self setUpSort:bar methods:methods parameters:parameters];
//AB    }
    [self checkDiskCache];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if (self.searchController.showsSearchResultsController){
        return ([self.filteredListContent count]>0);
    }
	else{
        return [[self.sections allKeys] count];
    }
...
//AB    if (!hideSearchBarActive || [scrollView isEqual:self.searchDisplayController.searchResultsTableView]) return;
    NSArray *paths;
...
-(void)searchWeb:(NSMutableDictionary *)item indexPath:(NSIndexPath *)indexPath serviceURL:(NSString *)serviceURL{
    ...
    NSString *query = [searchString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]];
...
//AB                             UISearchBarLeftButton *bar = (UISearchBarLeftButton *)self.searchDisplayController.searchBar;
//AB                             bar.storeWidth = viewWidth;
//AB                             if ([self collectionViewCanBeEnabled] == YES){
//AB                                 [bar showLeftButton:YES];
//AB                             }
//AB                             [bar layoutSubviews];
...
//                 }
                 ...
                 if (![fanartPath isEqualToString:@""]){
                     fanartURL = [NSString stringWithFormat:@"http://%@%@", serverURL, [fanartPath stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLFragmentAllowedCharacterSet]]];
                 }
                 NSString *filetype=@"";
...
//AB        bar.rightPadding = 26;
    }
//AB    [bar setSortButtonImage:sortAscDesc];
//AB    [bar layoutSubviews];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    self.searchController.searchBar.tintColor = [utils lighterColorForColor:searchBarColor];
    [channelListUpdateTimer invalidate];
    channelListUpdateTimer = nil;
}
...
//AB    }
 ...
 //AB       [self setUpSort:bar methods:methods parameters:parameters];
 //AB   }
 //AB   [bar setPlaceholder:NSLocalizedString(@"Search", nil)];
...
//AB    UISearchBarLeftButton *bar = (UISearchBarLeftButton *)self.searchDisplayController.searchBar;
//AB    [self showActionSheet:nil sheetActions:sortOptions item:item rectOriginX:bar.sortButton.center.x rectOriginY:bar.sortButton.center.y];
}
